...
public class DistanceUtils {
	...
	public static BigDecimal calculateDistance(BigDecimal lat1, BigDecimal lon1, BigDecimal lat2, BigDecimal lon2) {
		...
		// 대원거리 공식
		BigDecimal a = BigDecimal.valueOf(Math.sin(deltaLat.divide(new BigDecimal("2"), RoundingMode.HALF_UP).doubleValue()))
				.multiply(BigDecimal.valueOf(Math.sin(deltaLat.divide(new BigDecimal("2"), RoundingMode.HALF_UP).doubleValue())))
				.add(BigDecimal.valueOf(Math.cos(radLat1.doubleValue()))
						.multiply(BigDecimal.valueOf(Math.cos(radLat2.doubleValue())))
						.multiply(BigDecimal.valueOf(Math.sin(deltaLon.divide(new BigDecimal("2"), RoundingMode.HALF_UP).doubleValue())))
						.multiply(BigDecimal.valueOf(Math.sin(deltaLon.divide(new BigDecimal("2"), RoundingMode.HALF_UP).doubleValue()))));
